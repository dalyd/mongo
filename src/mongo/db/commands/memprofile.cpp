 ...
 * mem profiler linked into mongod.
 *
 ...
 *     { _memProfilerStart: { profileFilename: '/path/on/mongod-host.prof' } }
 *
...
namespace mongo {
    ...
    namespace {
        ...
        /**
         * Common code for the implementation of mem profiler commands.
         */
        class MemProfilerCommand : public Command {
        public:
            MemProfilerCommand( char const *name ) : Command( name ) {}
            ...
            virtual void addRequiredPrivileges(const std::string& dbname,
                ...
                ActionSet actions;
                actions.addAction(ActionType::memProfiler);
                out->push_back(Privilege(ResourcePattern::forClusterResource(), actions));
            }

            // This is an abuse of the global dbmutex.  We only really need to
            // ensure that only one memprofiler command runs at once; it would
            // be fine for it to run concurrently with other operations.
            virtual bool isWriteCommandForConfigServer() const { return true; }
        };

        /**
         * Class providing implementation of the _memProfilerStart command.
         */
        class MemProfilerStartCommand : public MemProfilerCommand {
        public:
            MemProfilerStartCommand() : MemProfilerCommand( commandName ) {}
        ...
        } memProfilerStartCommandInstance;

        /**
         * Class providing implementation of the _memProfilerStop command.
         */
        class MemProfilerStopCommand : public MemProfilerCommand {
        public:
            MemProfilerStopCommand() : MemProfilerCommand( commandName ) {}
            ...
            static char const *const commandName;
        } memProfilerStopCommandInstance;

        char const *const MemProfilerStartCommand::commandName = "_memProfilerStart";
        char const *const MemProfilerStopCommand::commandName = "_memProfilerStop";

        bool MemProfilerStartCommand::run( OperationContext* txn,
                                           std::string const &db,
                                           BSONObj &cmdObj,
                                           int options,
                                           std::string &errmsg,
        ...
        bool MemProfilerStopCommand::run( OperationContext* txn,
                                          std::string const &db,
                                          BSONObj &cmdObj,
                                          int options,
                                          std::string &errmsg,
